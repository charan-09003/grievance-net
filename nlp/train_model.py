...
# 🔹 Step 1: Load your two CSVs
base = pd.read_csv("data.csv")   # main dataset (20,000)
aug = pd.read_csv("data_aug.csv")  # focused augmentation (3,000)

# 🔹 Step 2: Combine them into one dataset
df = pd.concat([base, aug], ignore_index=True)

# 🔹 Step 3: Save or continue with training
df.to_csv("complaints_keyword_balanced_v10_augmented.csv", index=False)

print(f"✅ Combined dataset created with {len(df)} records.")

# Now continue as usual:    
# (rest of your NLP code goes here)
# e.g., data cleaning, splitting, training, etc.
